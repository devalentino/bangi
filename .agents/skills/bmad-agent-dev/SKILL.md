...
- Default scope is code/file editing only. Do not run builds, tests, linters, formatters, package-manager commands, servers, database commands, network operations, Git/GitHub actions, Jira/Confluence actions, or other operational commands unless the human explicitly asks for that action in the current turn.
- After editing, report what changed and state that verification was not run unless explicitly requested.
...
- Avoid patching and mocking when the scenario can be expressed through real inputs, fixtures, or persisted state. Treat `mock`, `patch`, and `monkeypatch` as a last resort rather than a default testing tool.
...
- Perform all request normalization at the request-schema/input-boundary layer only, using `pre_load`, `post_load`, or field configuration. Services must consume already-normalized values and must not normalize, coerce, sanitize, or infer around raw request data.
- Use explicit schema/serialization boundaries for all response shaping. Do not introduce route/service helpers whose job is to massage, normalize, mask, or repackage payloads unless the human explicitly asks for that abstraction.
- Assume database state is consistent even when model fields remain nullable for migration reasons; do not add read-path normalization, coercion, masking, or sentinel handling in services/helpers to compensate for historical schema transitions, invalid persisted data, or request-contract/output mismatches. If the selected request contract cannot be satisfied by the queried data, fail with a validation/error path instead of hiding the problem.
...
- Do not introduce new abstractions by default. If a change appears to require a new helper, serializer, flag, API field, config switch, wrapper, layer, protocol, or sentinel value, propose it briefly and wait for explicit human confirmation before implementing it. This especially applies to ad hoc service/helper methods whose only purpose is to reshape, normalize, mask, infer, or disambiguate request or response data.
...
- Do not patch service internals in tests when the same behavior can be verified through public inputs and persisted state.
...
- Keep responsibilities decoupled: services should return domain/report variables, while routes and schemas should assemble and validate the HTTP response representation according to local API conventions. If a value still needs interpretation after schema loading, the schema contract is wrong and must be fixed there rather than compensated for in the service.
