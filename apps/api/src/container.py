...
container = create_sync_container(
    parameters={
        ...
        'HEALTH_STALE_AFTER_SECONDS': _get_env('HEALTH_STALE_AFTER_SECONDS', int, 2 * 60 * 60),
        'HEALTH_WARNING_PERCENT': _get_env('HEALTH_WARNING_PERCENT', int, 70),
        'HEALTH_CRITICAL_PERCENT': _get_env('HEALTH_CRITICAL_PERCENT', int, 80),
    },
    services=[
        database,
        WorkerContext,
